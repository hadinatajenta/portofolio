    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div
        ...
        <div class="space-y-6 max-w-md">
          <div class="space-y-1">
            <p class="text-lg font-bold tracking-tight text-[var(--color-footer-text)]">{{ name }}</p>
            <p class="text-sm text-[var(--color-footer-text)]/50">{{ role }}</p>
          </div>
          <p class="text-sm leading-relaxed text-[var(--color-footer-text)]/60">{{ intro }}</p>
          <router-link to="/contact"
            class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[var(--color-footer-text)] text-[var(--color-footer-bg)] text-sm font-semibold transition-all duration-200 hover:opacity-90 hover:gap-3 active:scale-95">
            {{ ctaButtonLabel }}
        ...
        <div class="flex flex-col sm:flex-row gap-12">
          <!-- Quick Links -->
          <div class="space-y-4">
            <p class="text-[11px] font-bold uppercase tracking-widest text-[var(--color-footer-text)]/30">Navigation</p>
            <nav class="flex flex-col gap-2">
              <router-link v-for="menu in menus" :key="menu.index" :to="menu.route"
                class="text-sm text-[var(--color-footer-text)]/60 hover:text-[var(--color-footer-text)] transition-colors duration-200">
                {{ menu.name }}
              </router-link>
            </nav>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="flex flex-col gap-2">
              <a v-for="method in contactMethods" :key="method.id" :href="method.link" :target="method.target" :rel="method.rel"
                class="group text-sm text-[var(--color-footer-text)]/60 hover:text-[var(--color-footer-text)] transition-colors duration-200 flex items-center gap-2.5">
                <span class="flex items-center justify-center w-4 h-4 text-[var(--color-footer-text)]/70 group-hover:text-[var(--color-footer-text)] transition-colors flex-shrink-0">
                  <!-- GitHub -->
                  <svg v-if="method.id === 'github'" class="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  <EnvelopeIcon v-else-if="method.id === 'email'" class="w-4 h-4 stroke-[1.75]" aria-hidden="true" />
                  <!-- Fallback -->
