<template>
  <section class="space-y-4">
    <p class="text-xs font-bold uppercase tracking-widest text-[var(--color-text-tertiary)]">Hadinata Jenta · Engineering Portfolio</p>
    <h1 class="text-4xl sm:text-5xl font-bold leading-tight text-[var(--color-text)] max-w-2xl">
    ...
    <div class="flex flex-wrap gap-5 pt-4">
      <div v-for="stat in meaningfulStats" :key="stat.label" class="flex flex-col gap-1">
        <span class="text-2xl font-bold text-[var(--color-text)]">{{ stat.value }}</span>
        <span class="text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)]">{{ stat.label }}</span>
      </div>
    </div>
  </section>
</template>
