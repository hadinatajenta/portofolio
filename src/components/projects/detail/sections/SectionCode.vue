<template>
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
      <h2 class="text-lg font-bold text-[var(--color-text)]">Code Repositories</h2>
      <span class="text-xs text-[var(--color-text-muted)] font-medium">{{ items.length }} repo{{ items.length !== 1 ? 's' : '' }}</span>
    </div>

    <!-- Empty state -->
    <div v-if="items.length === 0" class="flex flex-col items-center justify-center py-16 gap-3 text-center rounded-xl border border-dashed border-[var(--color-border)]">
      <svg class="h-8 w-8 text-[var(--color-text-muted)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" />
      </svg>
      <p class="text-sm font-medium text-[var(--color-text-secondary)]">No public repositories</p>
...
const statusColor = (s) => {
  ...
  if (s === 'Completed') return 'text-[var(--color-text-muted)]'
  return 'text-amber-600 dark:text-amber-400'
}

const statusDot = (s) => {
  if (s === 'Active') return 'bg-emerald-500'
  if (s === 'Completed') return 'bg-[var(--color-border-strong)]'
  return 'bg-amber-400'
}
</script>
