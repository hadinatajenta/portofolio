<template>
  ...
  >
    ...
    <!-- Artifact Summary -->
    <p v-if="artifactSummary" class="mt-3 text-[11px] font-medium text-[var(--color-text-muted)] tracking-wide">
      {{ artifactSummary }}
    </p>
    ...
    <div class="mt-4 flex flex-wrap gap-1.5 pt-3 border-t border-[var(--color-border)]">
      <span
        ...
        :key="tech"
        class="rounded-full border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--color-text-secondary)] font-dot"
      >
        {{ tech }}
      </span>
      <span
        v-if="project.stack.length > 4"
        class="rounded-full border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-text-muted)] font-dot"
      >
